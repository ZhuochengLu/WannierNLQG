...
function _star_construction_quality_gate!(
    ...
    action =
        passed ? "CONTINUE" : config.construction_policy == :standard ? "CONTINUE_STANDARD" : "STOP"
    ...
    for target in context.star
        ...
        if symmetrized_hamiltonian !== nothing
            ...
            shifts_parent = [
                ...
                native_point_metadata(context.native, target).energies_ev[context.workspace[native_band]]
                for native_band in 1:workspace_count
            ]
...
function _star_frozen_target_complement_eigenpair(
    ...
    get(context, :construction_policy, :strict) == :standard ||
...
"""
function _build_star_covariant_paw_payload(
    config::SymmetryCovariantWavefunctionPreparationConfig,
    prepared_raw_native = nothing,
)
    ...
    raw_native = if prepared_raw_native === nothing
        _preparation_stage!(config, "native_read_started")
        data = _read_augmentation_aware_native_source(parent_source)
        preparation_release!()
        _preparation_stage!(config, "native_read_completed")
        data
    else
        prepared_raw_native
    end
    ...
    stage_contract = _preparation_contract(config, (; raw_native))
    raw_metric, native_norm, native_norm_worst =
        _preparation_cached_stage(config, stage_contract, "raw_metric") do
            _strict_sewing_metric(parent_source, raw_native)
        end
    preparation_release!()
    _preparation_stage!(config, "raw_metric_completed")
    ...
    lowdin = _preparation_cached_stage(config, stage_contract, "lowdin") do
        _star_lowdin_native(
            raw_native,
            raw_metric,
    ...
    preparation_release!()
    _preparation_stage!(config, "lowdin_completed")
    ...
    final_points = preparation_vector(PlaneWaveKPoint, "completed", nk)
    ...
    common = (;
        ...
        target_scope_active,
        symmetrized_target_authority,
        initial_maxima = copy(maxima),
        initial_contexts = copy(contexts),
    )
    execution_state = _preparation_star_execution(config, common, stars)
    for (star_index, star) in enumerate(stars)
        selected_star_indices !== nothing &&
            !(star_index in something(selected_star_indices)) &&
            continue
        result = _preparation_star_result!(execution_state, star_index, star)
        (;
            solution,
            representative,
            authority_indices,
        ...
        _merge_preparation_star_diagnostics!(config, maxima, contexts, result)
        extra_bands_per_star[star_index] = result.payload.extra_bands
        ...
        preparation_release!()
        _preparation_stage!(config, "star_completed"; star_index)
    ...
    diagnostic_controlled =
        ...
        (gauge.hamiltonian_correction::FarBandCovarianceCorrection).qualification_mode == :standard
    ...
    target_contract_sha256 = if config.target_subspace_contract === nothing
        ...
        authority_metadata["target_scope_production_eligible"] =
            config.construction_policy == :standard ? "false" : "true"
    ...
    preparation_release!()
    _preparation_stage!(config, "completed_metric_started")
    completed_metric, completed_norm, completed_norm_worst =
        _preparation_cached_stage(config, stage_contract, "completed_metric") do
    ...
    preparation_release!()
    _preparation_stage!(config, "replay_started")
    local_replay = if symmetrized_target_authority
        data = _preparation_cached_stage(config, stage_contract, "replay") do
    ...
    preparation_release!()
    _preparation_stage!(config, "parent_transport_audit_started")
    ...
    audit_payload = if symmetrized_authority
        _SymmetrizedDFTHamiltonianAuditPayload(
            hcat(
                (
                    native_point_metadata(raw_native, k).energies_ev for
                    k in eachindex(raw_native.kpoints)
                )...,
            ),
...
function _prepare_symmetry_covariant_wavefunctions_impl(
    config::SymmetryCovariantWavefunctionPreparationConfig,
)
    validate_symmetry_covariant_wavefunction_preparation_config(config)
    input_hashes =
        Dict(name => sha256_file(path) for (name, path) in _preparation_source_files(config.source))
    payload_contract =
        _preparation_contract(config, (; raw_native = (; input_sha256 = input_hashes)))
    # Pilot selection remains part of the binding; it cannot restore a full-grid run.
    payload_key = payload_contract * repr((config.maximum_star_count, config.selected_star_indices))
    payload = _preparation_cached_stage(config, payload_key, "completed_payload") do
        _preparation_stage!(config, "native_read_started")
        raw_native = _read_augmentation_aware_native_source(
            _star_source_with_band_range(config.source, nothing),
        )
        preparation_release!()
        _preparation_stage!(config, "native_read_completed")
        _build_star_covariant_paw_payload(config, raw_native)
    end
    preparation_release!()
    preparation_compact_workspace!(payload)
    _preparation_stage!(config, "strict_representation_started")
    diagnostics = String[]
    gauge = config.wavefunction_gauge_backend::StarCovariantPAWGauge
    standard_route =
    ...
    status = standard_route ? :STANDARD : :PASS
    ...
    try
        ...
        strict_contract = _preparation_contract(config, (; raw_native = payload.native))
        built = _preparation_cached_stage(config, strict_contract, "strict_representation") do
            build_augmentation_aware_band_representation(
                ...
                enforce_thresholds = !standard_route,
    ...
    output_path =
        ...
        status == :STANDARD ? splitext(config.output_hdf5)[1] * ".standard.h5" :
        splitext(config.output_hdf5)[1] * ".hold.h5"
    preparation_release!()
    _preparation_stage!(config, "gauge_write_started")
    output =
        _write_star_covariant_paw_gauge_hdf5(output_path, payload; status, root_cause, diagnostics)
    if status in (:PASS, :STANDARD)
