...
function build_config(;
    ...
    return TaskConfig(
        ...
        tasks = [
            TaskSpec(
                id = "kslice_zeeman_interband_berry_curvature_conventional",
                quantity = "ZIBCK",
                method = "Conventional",
                ...
                observable = KSliceSelection(
                    component = TensorComponent(2, 2),
                    bands = InterbandGroups(first = [3, 4], second = [1, 2]),
                ),
            ),
        ],
        ...
        output = OutputOptions(
            output_root = ExampleSupport.example_output_root(
                "kslice_zeeman_interband_berry_curvature_conventional",
                output_root,
            ),
