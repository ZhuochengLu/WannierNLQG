...
function build_config(;
    ...
    return TaskConfig(
        ...
        tasks = [
            TaskSpec(
                id = "kslice_triple_phase_product_conventional",
                quantity = "TPPK",
                method = "Conventional",
                physics = GeometryParameters(),
                numerics = GeometryNumerics(denominator_regularization = 0.001),
                observable = KSliceSelection(
                    component = TensorComponent(1, 2, 1),
                    bands = TripleGroups(first = [3], second = [4], third = [1, 2]),
                ),
            ),
        ],
        ...
        output = OutputOptions(
            output_root = ExampleSupport.example_output_root(
                "kslice_triple_phase_product_conventional",
                output_root,
            ),
