...
function build_config(;
    ...
    return TaskConfig(
        ...
        tasks = [
            TaskSpec(
                id = "integral_photon_drag_injection_current_conventional",
                quantity = "PDIC",
                method = "Conventional",
                physics = FiniteQOpticalParameters(
                    photon_energies = collect(range(0.0, 4.0; length = 200)),
                    fermi_energy = 0.0,
                    temperature = 0.0,
                    photon_momentum = (0.005, 0.0, 0.0), # Replace for the physical light field.
                ),
        ...
        output = OutputOptions(
            output_root = ExampleSupport.example_output_root(
                "integral_photon_drag_injection_current_conventional",
                output_root,
            ),
